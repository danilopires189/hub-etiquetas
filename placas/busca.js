...
async function loadData() {
  // 1. Verificar se BASE_BARRAS.js foi carregado
  if (window.DB_CADASTRO && window.DB_CADASTRO.BASE_CADASTRO) {
    console.log('📦 BASE_BARRAS.js:', window.DB_CADASTRO.BASE_CADASTRO.length, 'itens');
    processData(window.DB_CADASTRO.BASE_CADASTRO, true);
    return;
  }

  // 2. Cache como fallback
  try {
  ...
  // 3. Fallback para carregamento manual
  showManualLoad();
}
...
function renderHints(list) {
  ...
  list.forEach((r, i) => {
    ...
    div.innerHTML = `<span class="hint-cod">${highlight(r.CODDV || '', q)}</span>
...
function findMatches(q) {
  ...
  for (let i = 0; i < DATA.length; i++) {
    ...
    if (isDigits) {
      // Busca exata por código (prioridade máxima)
      if (cod === digits) score = 200;
      // Busca que começa com o código
      else if (cod.startsWith(digits)) score = 100 - Math.min(99, cod.length);
      // Busca exata por código de barras
      else if (barras === digits) score = 190;
      // Busca que começa com código de barras
      else if (barras.startsWith(digits)) score = 95 - Math.min(99, barras.length);
      // Busca que contém o código
...
function selectByIdx(i) {
  ...
  const q = $('#q');
  if (q) q.value = `${r.CODDV} / ${r.BARRAS} / ${r.DESC}`;
...
function renderCard(r) {
  ...
  const meta = $('#meta');
  if (meta) meta.textContent = `CODDV: ${r.CODDV}`;
...
function setupInterface() {
  ...
  if (btnC) btnC.addEventListener('click', () => {
    const r = DATA[CURRENT_IDX] || {};
    if (r.CODDV) copyToClipboard(String(r.CODDV));
  });
  ...
  if (btnReload) btnReload.addEventListener('click', async () => {
    ...
    // Tentar recarregar do BASE_BARRAS.js
    if (window.DB_CADASTRO && window.DB_CADASTRO.BASE_CADASTRO) {
      processData(window.DB_CADASTRO.BASE_CADASTRO, true);
    } else {
      await loadData();
