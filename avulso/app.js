  ...
  $('#gerar').addEventListener('click', async () => {
    ...
    if (success && window.supabaseManager && window.contadorGlobal && window.contadorGlobal.isSupabaseIntegrated()) {
      try {
        ...
        // Obter matrícula e nome validados
        const matriculaInput = $('#matricula');
        let matricula = matriculaInput.value.trim();
        let nome = ''; // Padrão vazio

        console.log('🔍 Iniciando busca de nome para matrícula:', matricula);

        // 1. Tentar pegar do sistema de validação (cache em memória)
        if (window.UserValidation && window.UserValidation.currentUser && String(window.UserValidation.currentUser.Matricula) === matricula) {
          nome = window.UserValidation.currentUser.Nome;
          console.log('✅ Nome encontrado via UserValidation:', nome);
        }

        // 2. Fallback: Busca direta na base (força bruta)
        if (!nome) {
          if (window.DB_USUARIO && window.DB_USUARIO.BASE_USUARIO) {
            console.log('⚠️ Nome não estava no cache, buscando em DB_USUARIO...');

            // Conversão segura para comparação
            const matBusca = parseInt(matricula, 10);

            // Tenta encontrar comparando como número e como string
            const usuarioEncontrado = window.DB_USUARIO.BASE_USUARIO.find(u =>
              u.Matricula == matBusca || String(u.Matricula).trim() === matricula
            );

            if (usuarioEncontrado) {
              nome = usuarioEncontrado.Nome;
              console.log('✅ Nome encontrado via busca direta (DB):', nome);
            } else {
              console.error('❌ CRÍTICO: Usuário não encontrado na base para matrícula:', matricula);
              console.log('   Exemplo de usuário na base:', window.DB_USUARIO.BASE_USUARIO[0]);
            }
          } else {
            console.error('❌ ERRO: Banco de usuários (window.DB_USUARIO) não carregado!');
          }
        }
        ...
        const tipoMovimentacao = tipoSelect.options[tipoSelect.selectedIndex].text;
        const volRaw = $('#volume').value;
        const volStr = padLeft(volRaw, 5);

        // Reconstruir o ID da movimentação para salvar corretamente
        const now = new Date();
        const mm = padLeft(String(now.getMonth() + 1), 2);
        const yy = String(now.getFullYear()).slice(-2);
        const calendario = `${mm}${yy}`;
        ...
        const avulsoData = {
          id_mov: codigoDotted,
          cd: depo, // Depósito original selecionado
        ...
        await window.supabaseManager.saveAvulsoLabel(avulsoData);
        console.log('✅ Geração de avulso registrada no Supabase com sucesso');
      } catch (error) {
