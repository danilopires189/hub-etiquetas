  ...
  $('#gerar').addEventListener('click', async () => {
    ...
    if (window.supabaseManager) {
      try {
        ...
        // Calcular total de etiquetas geradas (reutilizando lógica)
        let totalLabels = 0;
        if (labelType === 'external') {
          totalLabels = qtd * copias;
        } else if (labelType === 'internal') {
          ...
          totalLabels = (qtd * copias) + (qtd * 2); // Ajustado conforme lógica visual: Externa(copias) + Interna(1?) - Revisar lógica original
          // IMPORTANTE: Na lógica de geração (gerar()):
          // both -> (qtd * copias) [EXTERNAL] + (qtd * copias) [INTERNAL]
          // Espera, no código original:
          // if (labelType === 'both') { totalOperations = (qtd * copias) + (qtd * copias); }
          // Mas no updateCalculatedInfo estava: totalLabels = (qtd * copias) + (qtd * 2); ???
          // Vamos seguir a lógica do gerar() que é a real:
          totalLabels = (qtd * copias) * 2;
        }

        // Calcular número final
        const baseNum = parseInt(base, 10);
        const finalNum = baseNum + qtd - 1;
        // Padding simples para manter tamanho original
        const numFinalFormatted = (Array(base.length + 1).join('0') + finalNum).slice(-base.length);
        ...
        const tipoPT = tipoMap[labelType] || labelType;

        const caixaData = {
          num_inicial: base,
          num_final: numFinalFormatted,
          qtd: qtd,
          copia: copias,
          total_et: totalLabels,
          tipo: tipoPT
        };

        await window.supabaseManager.saveCaixaLabel(caixaData);
        console.log('✅ Geração de caixa registrada no Supabase com detalhes');
      } catch (error) {
