    ...
    if (window.supabaseManager && window.contadorGlobal && window.contadorGlobal.isSupabaseIntegrated()) {
      try {
        ...
        if (tipo === 'pulmao') {
          ...
          const colIni = parseInt($('#colIni').value || '1', 10);
          const colFim = parseInt($('#colFim').value || '1', 10);
          const niveis = $$('.nivel').filter(n => n.checked).length;
          
          const prefixes = prefixFim ? Math.abs(parseInt(prefixFim.match(/\d+$/)?.[0] || '0') - parseInt(prefixIni.match(/\d+$/)?.[0] || '0')) + 1 : 1;
          const colunas = colFim - colIni + 1;
          quantity = prefixes * colunas * (niveis || 1);
        } else if (tipo === 'estacao') {
          const prefixIni = $('#prefixIniE').value;
          const prefixFim = $('#prefixFimE').value;
          quantity = prefixFim ? Math.abs(parseInt(prefixFim.match(/\d+$/)?.[0] || '0') - parseInt(prefixIni.match(/\d+$/)?.[0] || '0')) + 1 : 1;
        } else if (tipo === 'outro') {
          // Estimativa baseada nos campos preenchidos
          quantity = 1; // Valor padrão para tipo outro
        }
        ...
        console.log('✅ Geração de endereçamento registrada no Supabase');
      } catch (error) {
